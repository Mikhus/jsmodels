...
class Observer {
     ...
     * Errors would be written to a given errors container, emitting
    ...
    static merge(target, source, schema, errors, subscriber, propPath = null) {
        ...
        let sourceType = schema.typeOf(source);
        let sameType = sourceType === schema.type;
        let isStrict = schema.options.strict;

        if (!sameType) {
            raiseError(errors, subscriber, schema, propPath, source, isStrict,
                Observer.ERROR_TYPE, schema.type, sourceType);

            Log.debug('Attempt to assign invalid data to a root data of ' +
                'the model:',
                JSON.stringify(errors[errors.length - 1], null, 2)
            );
            ...
            if (sourceLength <= targetLength) {
                ...
                for (; i < sourceLength; i++) {
                    if (primitive) {
                        ...
                        // new array element from source data
                        target[i] = Observer.observe(
                            source[i],
                            schema.items,
                            errors,
                            subscriber,
     ...
     * definition of the given schema, which should be associated with given
     * object.
    ...
    static observe(data, schema, errors, subscriber, propPath = null) {
        ...
        let handler = {
            get(target, property) {
                ...
                console.log('array change:', property);
                ...
                return function() {
                    ...
                    for (; i < s; i++) {
                        ...
                        if (!Observer.options.allowInvalid &&
                            !validate(args[i], schema.items, errors, subscriber,
                                path + property + '.' + i)) {
                            args.splice(i, 1);
                        }
                    }
                    ...
                    if (args.length) {
                        ...
                        subscriber.emit('change', property, target, args);
                    }
            ...
            set(target, property, value) {
                ...
                if (isArray) {
                    if (!isNumeric(property) || isObserved(value)) {
                        ...
                        subscriber.emit('change', property, target, value);
            ...
            deleteProperty(target, property) {
                ...
                subscriber.emit('change', property, target);
