 ...
 * @access private
 */
function raiseError(errors, subscriber, shouldThrow, code, ...templateVars) {
...
class Observer {
    ...
    static observe(data, schema, errors, subscriber) {
        if (!(schema instanceof Schema)) {
            ...
            raiseError(errors, subscriber, true, Observer.ERROR_SCHEMA);
        }

        if (!(subscriber instanceof EventEmitter)) {
            // invalid subscriber interface provided - throw error
            raiseError(errors, subscriber, true, Observer.ERROR_LISTENERS);
        }
