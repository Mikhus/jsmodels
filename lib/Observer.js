...
class Observer {
     ...
     * @returns {Proxy|*}
     */
    static observe(obj, schema, identifiable = true, parent = null) {
        ...
        if (!isValid && isStrict) {
            throw new TypeError(
                'Data model validation error: Given data is invalid!');
        }
    ...
    /*
    our goal to have something like this:

    schema = { // this is like design for our data
        'firstName': String,
        'lastName': String(''),
        '?email': {
            type: 'string',
            validator: email => /^.*?@.*?\.com$/.test(email)
        },
        'addresses': Array.of({
            'street': String,
            'city': String,
    ...
    data = Observer.observe({ // this is our actual data
        "firstName": "John",
        "lastName": "Doe",
        "addresses": [{
            "street": "350 5th Ave",
    ...
    data.firstName = 15;
    delete data.addresses[0].city;
    data.email += 'on';

    errors(data) = { // this is what we got when data changed
        firstName: type error,
        addresses: {
            0: {
                city: required error
            }
    ...
    */
    ...
    /**
     * Returns top root supervised data object for the given object.
     * So it means given object is a part of data of the top-root object.
     * If the given supervised object does not have parent it will be simply
     * returned back.
     *
     * @param {Proxy|*} supervised
     * @returns {Proxy|*}
     */
    static root(supervised) {
        if (!metadata.has(supervised)) {
            return supervised;
        }

        let parent;

        while (parent = metadata.get(supervised).parent) {
            supervised = parent;
