...
/**
 * Construct and returns full path of the property for the given schema
 *
 * @param {Schema} schema
 ...
 * @access private
 */
function fullPath(schema, property = null) {
    let parent = schema;
    let path = property || '';
 ...
 * @access private
 */
function raiseError(errors, subscriber, schema, property, value, shouldThrow,
                    code, ...vars)
{
    ...
    let error = {
        ...
        varName: schema.name || (schema.parent && schema.parent.type === 'array'
            ? schema.parent.name : ''),
        varValue: value,
        varPath: fullPath(schema, property)
    };
 ...
 * @access private
 */
function validate(data, schema, errors, subscriber, property = null) {
    markValidated(data);
    ...
    if (schema.required && data === void 0) {
        raiseError(errors, subscriber, schema, property, data, isStrict,
        ...
        for (; i < s; i++) {
            isValid = isValid && validate(data[i], schema.items, errors,
                subscriber, property);
        }
    }
        ...
        for (let i = 0, s = props.length; i < s; i++) {
            ...
            if (!(prop in schema.properties)) {
                raiseError(errors, subscriber, schema, prop, data[prop],
        ...
        for (let i = 0, s = props.length; i < s; i++) {
            ...
            if (propSchema.required) {
                isValid = isValid && validate(data[prop], propSchema,
                    errors, subscriber);
            }
        }
    }
...
class Observer {
     ...
     * @returns {Proxy|*}
     */
    static observe(data, schema, errors, subscriber, property = null) {
        ...
        if (!isValidated(data)) {
            validate(data, schema, errors, subscriber, property);
        }
        ...
        let handler = {
            ...
            set(target, property, value) {
                if (isArray) {
                    ...
                    if (!validate(value, schema.items, errors, subscriber, property)) {
                        return ;
                    }
                }
                ...
                if (isObject) {
                    if (property in schema.properties) {
                        ...
                        if (!validate(value, schema.properties[property],
                                errors, subscriber)
                        ) {
                            return ;
                        }
                    ...
                    raiseError(errors, subscriber, schema, property, value,
                        isStrict, Observer.ERROR_UNEXPECTED, property);
