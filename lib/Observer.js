 ...
 * @access private
 */
function raiseError(errors, subscriber, name, value, shouldThrow, code,
                    ...templateVars)
{
 ...
 * @access private
 */
function validate(data, schema, errors, subscriber) {
    markValidated(data);
    ...
    if (schema.required && data === void 0) {
        raiseError(errors, subscriber, schema.name, data, isStrict,
        ...
        for (; i < s; i++) {
            isValid = isValid && validate(data[i], schema.items,
                errorsOf(errors, i), subscriber);
        }
    }
        ...
        for (let i = 0, s = props.length; i < s; i++) {
            ...
            if (!(prop in schema.properties)) {
                raiseError(errorsOf(errors, prop), subscriber, prop, data[prop],
        ...
        for (let i = 0, s = props.length; i < s; i++) {
            ...
            if (propSchema.required) {
                isValid = isValid && validate(data[prop], propSchema,
                    errorsOf(errors, prop), subscriber);
            }
        }
    }
...
class Observer {
     ...
     * @returns {Proxy|*}
     */
    static observe(data, schema, errors, subscriber) {
        if (!(schema instanceof Schema)) {
            // invalid schema interface provided - throw error!
            raiseError(errors, subscriber, schema.name, data, true,
        ...
        if (!isValidated(data)) {
            validate(data, schema, errors, subscriber);
        }
        ...
        let handler = {
            ...
            set(target, property, value) {
                if (isArray) {
                    ...
                    if (!validate(value, schema.items, errors, subscriber)) {
                        return ;
                    }
                }
                ...
                if (isObject) {
                    if (property in schema.properties) {
                        ...
                        if (!validate(
                                ...
                                errorsOf(errors, property), subscriber)
                        ) {
                            return ;
                        }
                    }

                    raiseError(errorsOf(errors, property), subscriber,
                        property, value, isStrict, Observer.ERROR_UNEXPECTED,
                        property);
